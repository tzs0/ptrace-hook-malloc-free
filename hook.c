...
#define hook_printf(fmt, ...) 		//printf("[%s %d] "fmt, __func__, __LINE__, ##__VA_ARGS__)
...
static int check_elfheader(uintptr_t base_addr)
{
	...
	if(ELFCLASS64 != ehdr->e_ident[EI_CLASS]) {
		hook_printf("ehdr->e_ident[EI_CLASS](%d)!=ELFCLASS64(%d) \n", ehdr->e_ident[EI_CLASS], ELFCLASS64);
		return -1;
	}
#else
	if(ELFCLASS32 != ehdr->e_ident[EI_CLASS]) {
		hook_printf("ehdr->e_ident[EI_CLASS](%d)!=ELFCLASS32(%d) \n", ehdr->e_ident[EI_CLASS], ELFCLASS32);
		return -1;
	}
#endif
	...
	if(ELFDATA2LSB != ehdr->e_ident[EI_DATA]) {
		hook_printf("ehdr->e_ident[EI_DATA](%d)!=ELFDATA2LSB(%d)\n", ehdr->e_ident[EI_DATA],ELFDATA2LSB);
		return -1;
	}
...
//	}
	...
	hook_printf("check_elfheader OK!\n");
...
{
	if(0 == self->base_addr) {
		hook_printf("error, base_addr == 0\n");
		return 1;
	}
	if(0 == self->bias_addr) {
		hook_printf("error, bias_addr == 0\n");
//        return 1; /* if ASLR is disable, bias_addr maybe 0 */
	}
	if(NULL == self->ehdr) {
		hook_printf("error, ehdr == NULL\n");
		return 1;
	}
	if(NULL == self->phdr) {
		hook_printf("error, phdr == NULL\n");
		return 1;
	}
	if(NULL == self->strtab) {
		hook_printf("error, strtab == NULL\n");
		return 1;
	}
	if(NULL == self->symtab) {
		hook_printf("error, symtab == NULL\n");
		return 1;
	}
	if(NULL == self->bucket) {
		hook_printf("error, bucket == NULL\n");
		return 1;
	}
	if(NULL == self->chain) {
		hook_printf("error, chain == NULL\n");
		return 1;
	}
	if(1 == self->is_use_gnu_hash && NULL == self->bloom) {
		hook_printf("error, bloom == NULL\n");
		return 1;
	}
...
static void elf_dump_dynamic(elf_info_t* self)
{
	...
	hook_printf("Dynamic section contains %zu entries:\n", dyn_cnt);
...
static void elf_dump_rel(elf_info_t* self, const char* type, ElfW(Addr) rel_addr, ElfW(Word) rel_sz)
{
	...
	hook_printf("Relocation section '.rel%s%s' contains %u entries:\n",
...
static void elf_dump_symtab(elf_info_t* self)
{
	...
	hook_printf("Symbol table '.dynsym' contains %u entries:\n", symtab_cnt);
	printf("  %-8s " \
	       "%-"UTIL_FMT_FIXED_S" " \
	       "%s\n",
	       "Idx",
...
//    if(log_priority < ANDROID_LOG_DEBUG) return;
	...
	hook_printf("Elf Pathname: %s\n", self->pathname);
	hook_printf("Elf bias addr: %p\n", (void*)self->bias_addr);
...
static int elf_init(elf_info_t* self, uintptr_t base_addr, const char* pathname)
{
	...
	if(NULL == phdr0) {
		hook_printf("Can NOT found the first load segment. %s\n", pathname);
		return -1;
	}

#if ELF_DEBUG
	if(0 != phdr0->p_vaddr)
		hook_printf("first load-segment vaddr NOT 0 (vaddr: %p). %s\n",
		            (void*)(phdr0->p_vaddr), pathname);
	else {
		hook_printf("phdr0->p_vaddr is 0\n");
	}
#endif

	hook_printf("self->base_addr:%#x phdr0->p_vaddr:%p\n", self->base_addr, phdr0->p_vaddr);
	//save load bias addr
	if(self->base_addr < phdr0->p_vaddr) {
		hook_printf("error, self->base_addr(%d)<phdr0->p_vaddr(%d)", self->base_addr, phdr0->p_vaddr);
		return -1;
	}

	self->bias_addr = self->base_addr - phdr0->p_vaddr;
	hook_printf("self->bias_addr：%#x \n", self->bias_addr);

	//find dynamic-segment
	ElfW(Phdr) *dhdr = elf_get_first_segment_by_type(self, PT_DYNAMIC);
	if(NULL == dhdr) {
		hook_printf("Can NOT found dynamic segment. %s\n", pathname);
		return -1;
	}
	hook_printf("PT_DYNAMIC:%d dhdr:%p dhdr->p_vaddr:%#x\n", PT_DYNAMIC, dhdr, dhdr->p_vaddr);

	//parse dynamic-segment
	self->dyn          = (ElfW(Dyn)*)(self->bias_addr + dhdr->p_vaddr);
	hook_printf("self->dyn:%#x\n", self->dyn);
	self->dyn_sz       = dhdr->p_memsz;
	hook_printf("self->dyn_sz:%d\n", self->dyn_sz);
	ElfW(Dyn) *dyn     = self->dyn;
	hook_printf("dyn:%p\n", dyn);

	ElfW(Dyn) *dyn_end = self->dyn + (self->dyn_sz / sizeof(ElfW(Dyn)));
	hook_printf("( self->dyn_sz / sizeof( ElfW( Dyn ) ) ):%d dyn_end:%p\n",
	            (self->dyn_sz / sizeof(ElfW(Dyn))),dyn_end);
	uint32_t*  raw;
	for(; dyn < dyn_end; dyn++) {
		if(dyn->d_tag>DT_NUM) {
			hook_printf("dyn:%p dyn->d_tag:%#x\n", dyn, dyn->d_tag);
		}
		else {
			hook_printf("dyn:%p dyn->d_tag:%d\n", dyn, dyn->d_tag);
		}
...
//				self->symtab = ( ElfW( Sym )* )( self->bias_addr + dyn->d_un.d_ptr );
				...
				if((ElfW(Addr))(self->symtab) < self->base_addr) {
					hook_printf("error, ( ElfW( Addr ) )( self->symtab )(%d)<self->base_addr(%d)\n",
...
//				self->relplt = ( ElfW( Addr ) )( self->bias_addr + dyn->d_un.d_ptr );
			...
			case DT_PLTRELSZ:
				...
				hook_printf("self->relplt_sz:%d\n", self->relplt_sz);
				break;
			case DT_REL:
				hook_printf("DT_REL:%d\n", DT_REL);
			case DT_RELA: {
				hook_printf("DT_RELA:%d\n", DT_RELA);
				hook_printf("DT_RELA bias_addr:%p dyn->d_un.d_ptr:%p\n",self->bias_addr,dyn->d_un.d_ptr);
//				self->reldyn = ( ElfW( Addr ) )( self->bias_addr + dyn->d_un.d_ptr );
				...
				if((ElfW(Addr))(self->reldyn) < self->base_addr) {
					hook_printf("error, ( ElfW( Addr ) )( self->reldyn )(%d)<self->base_addr(%d)\n",
			...
			case DT_ANDROID_RELA: {
				...
				if((ElfW(Addr))(self->relandroid) < self->base_addr) {
					hook_printf("error, ( ElfW( Addr ) )( self->relandroid )(%d)<self->base_addr(%d)\n",
...
//				raw = ( uint32_t* )( self->bias_addr + dyn->d_un.d_ptr );
			...
			case DT_GNU_HASH: {
				hook_printf("DT_GNU_HASH:%d bias_addr:%p dyn->d_un.d_ptr:%p\n",
...
//				raw = ( uint32_t* )( self->bias_addr + dyn->d_un.d_ptr );
	...
	if(0 != self->relandroid) {
		...
		if(self->relandroid_sz < 4 ||
		   ...
		   rel[2] != 'S' ||
		   rel[3] != '2') {
			hook_printf("android rel/rela format error\n");
			return -1;
		}
	...
	//check elf info
	if(0 != elf_check(self)) {
		hook_printf("elf init check failed. pathname:%s\n", pathname);
		return -1;
	}
	...
	hook_printf("init OK: %s (%s %s PLT:%u DYN:%u ANDROID:%u)\n", self->pathname,
	...
	for(i = self->bucket[hash % self->bucket_cnt]; 0 != i; i = self->chain[i]) {
		...
		if(0 == strcmp(symbol, symbol_cur)) {
			...
			*symtab = &(self->symtab[i]); /* 找个这个函数的符号表 */
			hook_printf("found %s at symidx: %u (ELF_HASH) symtab：%p\n", symbol, *symidx, *symtab);
			return 0;
		}
	}

	hook_printf("dont find\n");
	return -1;
}
	...
	while(1) {
		...
		if((hash | (uint32_t)1) == (symhash | (uint32_t)1) && 0 == strcmp(symbol, symname)) {
			...
			*symtab = &(self->symtab[i]);
			hook_printf("found %s at symidx: %u (GNU_HASH DEF) symtab:%p\n", symbol, *symidx, *symtab);
...
static int elf_gnu_hash_lookup_undef(elf_info_t* self, const char* symbol, uint32_t* symidx, ElfW(Sym)*  *symtab)
{
	...
	for(i = 0; i < self->symoffset; i++) {
		...
		if(0 == strcmp(symname, symbol)) {
			*symidx = i;
			*symtab = &(self->symtab[i]);
			hook_printf("found %s at symidx: %u (GNU_HASH UNDEF) symtab:%p\n", symbol, *symidx, *symtab);
			return 0;
		}
	}
                                     ...
                                     int* found)
{
	...
	if(is_plt && r_type != ELF_R_GENERIC_JUMP_SLOT) {
		hook_printf("is_plt(%d) && r_type(%d) != ELF_R_GENERIC_JUMP_SLOT(%d)\n", is_plt,r_type,ELF_R_GENERIC_JUMP_SLOT);
		return 0;
	}
	if(!is_plt && (r_type != ELF_R_GENERIC_GLOB_DAT && r_type != ELF_R_GENERIC_ABS)) {
		hook_printf("!is_plt(%d) && (r_type(%d) != ELF_R_GENERIC_GLOB_DAT(%d) && r_type != ELF_R_GENERIC_ABS(%d)\n", is_plt,r_type,ELF_R_GENERIC_GLOB_DAT,ELF_R_GENERIC_ABS);
		return 0;
	}

	//we found it
	hook_printf("found %s at %s offset: %p %p\n", symbol, section, (void*)r_offset, r_offset);
	if(NULL != found) {
		*found = 1;
	}

	//do replace
	hook_printf("self->bias_addr: %p\n", self->bias_addr);
	addr = self->bias_addr + r_offset; /* 指向GOT里需要重定位的地址 */
	hook_printf("symbol:%s in GOT addr:%#x %p\n", symbol, addr, addr);
	if(addr < self->base_addr) {
		hook_printf("addr(%#x) < self->base_addr(%#x)\n", addr,self->base_addr);
		return -1;
	}
	if(0 != (r = elf_replace_function(self, symbol, addr, new_func, old_func))) {
		hook_printf("replace function failed: %s at %s\n", symbol, section);
		return r;
	}
	...
	if((self->group_flags & RELOCATION_GROUP_HAS_ADDEND_FLAG) &&
	   (self->group_flags & RELOCATION_GROUPED_BY_ADDEND_FLAG)) {
		if(0 == self->is_use_rela) {
			hook_printf("unexpected r_addend in android.rel section\n");
			return -1;
		}
...
static int elf_replace_function(elf_info_t* self, const char* symbol, ElfW(Addr) addr, void* new_func, void** old_func)
{
	...
	if(*(void**)addr == new_func) {
		hook_printf("same, *(void **)addr:%p new_func:%p\n", *(void**)addr,new_func);
		return 0;
	}

	//get old prot
	if(0 != (r = util_get_addr_protect(addr, self->pathname, &old_prot))) {
		hook_printf("get addr prot failed. ret: %d\n", r);
		return r;
	}
	...
	if(old_prot != need_prot) {
		...
		if(0 != (r = util_set_addr_protect(addr, need_prot))) {
			hook_printf("set addr prot failed. ret: %d\n", r);
			return r;
		}
	}
	...
	//save old func
	old_addr = *(void**)addr;
	hook_printf("symbol:%s old_addr:%p\n", symbol, old_addr);
	...
	*(void**)addr = new_func;  //segmentation fault sometimes
	...
	if(old_prot != need_prot) {
		...
		if(0 != (r = util_set_addr_protect(addr, old_prot))) {
			hook_printf("restore addr prot failed. ret: %d\n", r);
		}
	}
...
void* hk_show_sym_info(int pid, const char* sym_name, const char* so_name)
{
	...
	hook_printf("find symbol:%s symidx:%d dynsym.st_value = %p, dynsym.size = %p\n",
	            sym_name, symidx, (symtab)->st_value, (symtab)->st_size);
//        hook_printf("old_func:%p new_func:%p\n", *(hook_info[i].old_func),(hook_info[i].new_func));
	void* ret = (elf_info.bias_addr+(symtab)->st_value);
	hook_printf("bias_addr(%p)+(symtab)->st_value(%p):[ %p ] [%s] in [%s]\n",
...
int hk_hook(void* elf, const char* sym, FUNC_PROXY proxy)
{
	...
	if(0 != self->relplt) {
		...
		hook_printf("self->relplt:%p self->relplt_sz:%d\n", self->relplt,self->relplt_sz);
		/* 遍历.rel(a).plt */
